...
class ArticleCategory extends AdminBase
{
    ...
    public function index(Request $request)
    {
        ...
        if($post_name==!''){
            ...
            $data_list = $category_data->where(['status'=>1,'site_id'=>$this->site_id]) -> order('sort','asc')-> select();
        }
    ...
    public function edit($id)
    {
        ...
        // 上级分类
        $parent_data = ArticleCategoryModel::get($data_list['parent_id']);
        if(empty($parent_data)){
            $data_list['parent_title'] = '顶级分类';
        }else{
            $data_list['parent_title'] = $parent_data['title'];
        }
