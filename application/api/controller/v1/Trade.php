...
class Trade extends Controller
{
    ...
    {
        $account_list = array();
        $demo_info_sql['status'] = 1;
        $demo_list = TradeAccountDemo::where($demo_info_sql) -> select();
    ...
    public function server()
    {
        ...
        $platform_data = array();
        $demo_info_sql['status'] = 1;
        $trade_server_info = TradeServer::where($demo_info_sql) -> select();
        foreach ($trade_server_info as $user) {
            $platform_data[] = $user['platform'];
        }

        // 过滤重复的平台名称，并重新排序
        $platform_arr = array_unique($platform_data);
        $platform_arr=array_merge($platform_arr);
        $platform_count=count($platform_arr);

        $platform_data=array();
        for($i=0;$i<$platform_count;$i++){
            $platform_name=array();
            $platform_name_sql['platform'] = $platform_arr[$i];
            $platform_name_info = TradeServer::where($platform_name_sql) -> select();
            foreach ($platform_name_info as $user) {
                $platform_name[] =$user['server_name'];

            }
            $platform_data[]=array("platform_name" => $platform_arr[$i],"platform_server" => $platform_name);
        }

        $json_info = array("code" => "1", "status" => "query success", "platform_data" => $platform_data);
    ...
    // 最大回撤率 DrawDown
    public function draw_down(Request $request)
    {
        $post_account = $request->post('account');
        if($post_account==''){
            $json_data = array("code" => "0", "status" => "account is null");
            return json_encode($json_data);
        }
        $account_info_sql['account'] = $post_account;
        $account_info = TradeAccount::where($account_info_sql)->find();
        if($account_info==''){
            $json_data = array("code" => "0", "status" => "account is error");
            return json_encode($json_data);
        }
        $aid = $account_info->id;
        // 获取 DrawDown
        $object_account = new DrawDown();
        $draw_down=$object_account->index($aid);
        echo $draw_down;
    ...
    public function account_profit(Request $request)
    {
        ...
        if($total_list==''){
            $json_data = array("code" => "0", "status" => "when order_type=6,Trade Order is null");
            return json_encode($json_data);
        }
        ...
        if($info_list==''){
            $json_data = array("code" => "0", "status" => "when order_type=0 and order_type=0, Trade Order is null");
            return json_encode($json_data);
        }
    ...
    {
        $post_account = $request->post('account');
        // post_account 为空，则返回错误提示
        if($post_account==''){
            $json_data = array("code" => "0", "status" => "The Account is null");
            return json_encode($json_data);
        }
        $account_info_sql['account'] = $post_account;
        $account_info = TradeAccount::where($account_info_sql)->find();
        // 找不到当前的 account ,则返回错误提示
        if($account_info==''){
            $json_data = array("code" => "0", "status" => "The Account is null");
            return json_encode($json_data);
        }
